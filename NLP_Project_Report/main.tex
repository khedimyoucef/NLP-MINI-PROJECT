...
Github repo link (source code) : https://github.com/khedimyoucef/NLP-MINI-PROJECT
\end{center}
...
we can note that the loss was a bit fluctuating over the steps but it still was decreasing overall, which is a good sign that the model was learning to adapt to the new task. 
The fluctuations could be due to the small batch size and the complexity of the task, but the downward trend indicates that the fine-tuning process was effective
in improving the model's performance on generating children's stories.
...
Another problem we faced was the management of the kaggles limited ram and storage resources so we had to implement a pipeline that constantly 
monitored and cleaned up the unnecessary files at/after each stage of the conversion and quantization process to ensure that we didn't run out of space during the process.
\begin{figure}[H]
\centering
...
\label{fig:gguf}
\end{figure}
the final GGUF file was then downloaded and tested on a local machine using \texttt{llama.cpp} to verify that the fine-tuning and quantization processes preserved 
the model's ability to generate coherent, age-appropriate stories.

and the resulting single gguf fine tuned model file was uploaded to this hugging face repository for presistency :https://huggingface.co/khedim/NLP-MINI-PROJECT/tree/main/gemma-e2b
\includegraphics[width=0.85\textwidth]{pics/gemma2HF.png}
while the finetuned gpt2 small is sitting alongside it in the same repo : https://huggingface.co/khedim/NLP-MINI-PROJECT/tree/main

\includegraphics[width=0.85\textwidth]{pics/gpt2-smallHF.png}
and the standalone adpater weights alongside the checkpint are also available here : https://huggingface.co/khedim/NLP-MINI-PROJECT-adapter/tree/main
\includegraphics[width=0.85\textwidth]{pics/adapterWeightsHF.png}   
\clearpage
\newpage
...
\begin{figure}[H]
\centering
example prompt : write a story about a disobedient kid who does want to study and ends up failing school
(note that we used "write a story" because we used the IT version of the model, which is already instruction tuned, 
+ our dataset was also formatted in the instruction format, so we wanted to keep the same format for testing as well to get the best results possible from the model)

\includegraphics[width=0.9\textwidth]{pics/GemmaE2Btest11.png}
\includegraphics[width=0.9\textwidth]{pics/GemmaE2Btest12.png}

and for the ages 11 to 12 for the same prompt :
...
from the results here we can see that not only the model was able to successfully use more complex language and grammer for the older age group 
but also made the ending in the 3-4 age group more positive and simple while the ending in the 11-12 age group was more realistic and complex, 
which shows that the model was able to learn the different storytelling styles that suit the differences of emotional and intellectual development between the two age groups
...
\end{minipage}\hspace{.2in}
\begin{minipage}{0.25\textwidth}\centering
\includegraphics[width=0.5\textwidth]{pics/tech/llama.png} 
\end{minipage}
...
add more technologies here : i have react.png , docker.png , fastapi.png , github.png , huggingface.png here in the same tech folder 


also state how to run (reproduce the full project locally) for the prof to try it herself on the cpu locally since we did not deploy it in the web
(how to setit up from 0 to 100 and run the model locally using llama.cpp and the gguf file that we uploaded to hugging face)
\end{minipage}
